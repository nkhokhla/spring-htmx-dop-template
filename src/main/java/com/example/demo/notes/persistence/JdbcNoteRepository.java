...
/**
 * Storage conventions (SQLite): ids as UUID text, timestamps as ISO-8601 UTC text
 * (which sorts chronologically as plain text) — both converted here and nowhere else.
 */
@Repository
class JdbcNoteRepository implements NoteRepository {
    ...
    public void save(Note note) {
        jdbc.sql("insert into note (id, text, created_at) values (:id, :text, :createdAt)")
                ...
                .param("createdAt", note.createdAt().toString())
                .update();
    }
